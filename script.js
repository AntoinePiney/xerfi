...
shaderParams = {
  ...
  chromaticMultiplierRed: 1.1,
  chromaticMultiplierBlue: 1.15,
...
async function init() {
  ...
  renderer.toneMappingExposure = 3.5; // Exposition encore plus élevée pour éclaircir davantage
...
// Configurer les contrôles OrbitControls optimisés
...
function initGUI() {
  ...
  renderFolder.add(shaderParams, 'cameraDistance', 5, 30, 0.5)
...
function updateCameraDistance() {
  camera.position.setLength(shaderParams.cameraDistance);
}
...
function animate(currentTime = 0) {
  ...
  // Contrôles sans damping - pas besoin d'update()
...
function animateCubes(cubeIndices, elapsed) {
  ...
  cubeIndices.forEach(index => {
    ...
    if (animationPhase === 1) {
      ...
      cube.scale.set(1, easedProgress, 1);
      ...
      cube.scale.set(1, 1, easedProgress);
      ...
      cube.scale.set(easedProgress, 1, 1);
