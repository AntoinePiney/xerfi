...
vec3 getVerticalIridescence(vec3 normal, vec3 viewDir, vec3 worldPos) {
  ...
  // Parameterized iridescent colors
  vec3 roseSubtle = vec3(0.5, 0.65, 0.9);    // Bleu rosé plutôt que rose
  ...
  // Add violet/rose tones more gradually for elegance
  if (fresnel > 0.6) {  // Commencer plus tard pour plus de subtilité
    float warmMix = (fresnel - 0.1) / 0.4;
    iridescent = mix(iridescent, violetSubtle, warmMix * 0.2);
    if (fresnel > 0.8) {
      float roseMix = (fresnel - 0.8) / 0.2;
      iridescent = mix(iridescent, roseSubtle, roseMix * 0.9);
    }
  }
...
void main() {
  ...
  // Enhanced color separation with violet chromatic aberrations
  vec3 chromaticColor = vec3(
    redSample.r * 0.45,   // Augmenter le rouge pour plus de violet
    greenSample.g * 0.65, // Réduire le vert pour moins de cyan
    blueSample.b * 1.35   // Bleu dominant avec nuance violette
  );
  ...
  vec3 edgeHighlight = vec3(0.4, 0.8, 1.0) * edgeDetection * 0.4; // Cyan ciel
  ...
  // Parameterized glass body color
  vec3 glassBodyColor = vec3(0.8, 0.92, 0.98) * glassBodyIntensity;
  ...
  // Enhance violet spectrum in chromatic aberrations
  finalColor.r = pow(finalColor.r, 1.2);  // Moins de réduction rouge pour violet
  finalColor.b = pow(finalColor.b, 0.75); // Bleu légèrement amplifié
  finalColor.g = pow(finalColor.g, 1.1);  // Réduire le vert pour moins de cyan
